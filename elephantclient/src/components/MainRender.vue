...
import {lngToPix, BASE_URL} from "../utils";
...
export default {
    ...
    mounted() {
        ...
        let renderAll = (position) => {
            ...
            let browserLat = lngToPix(
                position.coords.longitude
            );
            this.browserLat = browserLat;

            fetch(`${BASE_URL}/peer/${this.id}`).then(res => {
                if (res.ok) {
                    return res.json()
                }
            }).then(j => {
                ...
                this.frameInterval = setInterval(() => {
                    ...
                    window.scroll(
                        this.currentX - window.innerWidth/2, 0
                    )
                ...
                let reportLocation = () => {
                    ...
                    this.reportInterval = setTimeout(reportLocation, 1000);
                }
